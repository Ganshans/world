    ...
    bool OnGossipHello(Player* player, Creature* pCreature)
    {
        if (player && LOTTERY_ENABLE)
        {
            if (player->getLevel() >= uint32(LOTTERY_MINLVL) && player->GetMoney() >= uint32(LOTTERY_BETCOST))
            {
				...
				str << "Möchtest du dein Glück versuchen? Wähle " << LOTTERY_FIELDS << " Nummern zwischen 1 und " << LOTTERY_MAXNUMBER << " (separated by spaces), pay " << (uint32)(LOTTERY_BETCOST*0.0001f) << " gold and then wait for the draw (08:00pm).";
            ...
            {
                std::stringstream str;
				str << "Du musst Level " << LOTTERY_MINLVL << " sein und benötigst " << (uint32)(LOTTERY_BETCOST*0.0001f) << " Gold, um ein Ticket zu kaufen.";
				pCreature->MonsterWhisper(str.str().c_str(), player);
            }
        }
        return true;
        ...
        if (uiSender == GOSSIP_SENDER_MAIN)
        {
            switch (uiAction)
            {
                case GOSSIP_ACTION_INFO_DEF:
                {
					...
					error << "Du hast eine ungültige Nummer eingegeben. Du brauchst " << LOTTERY_FIELDS << " Nummern (mit Leerzeichen getrennt) zwischen 1 und " << LOTTERY_MAXNUMBER << ".";
                ...
                if (me->IsVisible())
                {
                    ...
                    if (qBets)
                    {
						...
						for(tdJW::const_iterator itr = lWinners.begin(); itr != lWinners.end(); itr++)
						{
							...
							text << "Glückwunsch! Du hast auf richtige Nummern getippt!\n\n";
                        ...
                        if (winnerCount[LOTTERY_FIELDS-1] > 0)
                        {
							...
							{
								std::stringstream text;
								text << "Unglaublich! Du hast den Jackpot gewonnen! Glückwunsch!\n\n";
